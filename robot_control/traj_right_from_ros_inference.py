...
class TrajFollow:
    def __init__(self, test_mode=False):
        ...
        # 4. rotate around loacal z for -30 degree
        self.R_hand_z_neg30 = np.array([
            [np.cos(-90*np.pi/180), -np.sin(-90*np.pi/180), 0],
            [np.sin(-90*np.pi/180), np.cos(-90*np.pi/180), 0],
            [0, 0, 1]
        ])
        self.R_hand_to_ee = np.dot(self.R_hand_to_ee, self.R_hand_z_neg30)
