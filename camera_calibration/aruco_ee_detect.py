...
class ArmKeyboardControl:
    def __init__(self, arm_choice):
        ...
        # rospy.Subscriber("/zedm/zed_node/left_raw/image_raw_color/compressed", CompressedImage, self.image_callback)
        ...
        try:
            ...
            with self.lock:
                if self.current_pose is not None:
                    ...
                    # T_base_cam = np.array([
                    #     [-0.34464716, -0.43693879, -0.83084465,  0.38650987],
                    #     [ 0.4611071 ,  0.69213953, -0.5552685 ,  0.19034064],
                    #     [ 0.81767877, -0.57448008, -0.03706834,  0.04365359],
                    #     [ 0.        ,  0.        ,  0.        ,  1.        ]
                    # ])
            ...
            rospy.logerr(f"Aruco detection or projection failed: {e}")
    ...
    def print_and_record(self):
        with self.lock:
            ...
            rospy.loginfo(f"[{self.arm}] Pose: pos({p.x:.3f}, {p.y:.3f}, {p.z:.3f})  ori({o.x:.3f}, {o.y:.3f}, {o.z:.3f}, {o.w:.3f})")
