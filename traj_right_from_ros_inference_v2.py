...
class TrajFollow:
    def __init__(self, mode='normal'):
        ...
        self.T_cam_to_base = np.array([
            [ 0.0685691,  -0.87717259,  0.47525418,  0.12518205],
            [-0.99729769, -0.07286137,  0.00940931, -0.01441062],
            [ 0.02637408, -0.47461508, -0.87979823,  0.45754432],
            [ 0.          ,0.          ,0.          ,1.        ]
        ])
        # Quaternion for camera to base rotation: [x, y, z, w]
        self.q_cam_to_base = np.array([0.71084962, -0.65923588, 0.17641856, -0.17022742])
        ...
        # Combined hand to ee transformation
        self.R_hand_to_ee = np.dot(self.R_hand_y_180, self.R_hand_z_90)

        self.R_hand_y_neg90 = np.array([
            [0, 0, 1],
            [0, 1, 0],
            [-1, 0, 0]
        ])
        self.R_hand_to_ee = np.dot(self.R_hand_to_ee, self.R_hand_y_neg90)

        # 4. rotate around local z for -90 degree
        self.R_hand_z_neg90 = np.array([
            [np.cos(-80*np.pi/180), -np.sin(-80*np.pi/180), 0],
            [np.sin(-80*np.pi/180), np.cos(-80*np.pi/180), 0],
            [0, 0, 1]
        ])
        self.R_hand_to_ee = np.dot(self.R_hand_to_ee, self.R_hand_z_neg90)
    ...
    def run_normal_mode(self):
        """Run normal trajectory following mode"""
        rate = rospy.Rate(20)
        rospy.loginfo(f"[{self.arm}] Starting trajectory with hand control at {rate} Hz...")
        self.send_vel_limit([4,4,4,4,4,4],[4,4,4,4,4,4])

        filepath = '/home/nvidia/ke/r1_pro_sdk/install/share/mobiman/script/right/right.csv'
    ...
    try:
        ...
        if len(sys.argv) > 1:
            if sys.argv[1] == '--move_step':
                ...
                print("Will follow commands from /inference/right/command topic")
                print("Expecting Float32MultiArray with format:")
                print("[wrist_x, wrist_y, wrist_z, qx, qy, qz, qw, hand_1, ..., hand_6]")
                print("=" * 50)
