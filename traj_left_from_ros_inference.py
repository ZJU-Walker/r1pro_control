...
class TrajFollow:
    def __init__(self, test_mode=False):
        rospy.init_node('traj_follow_left_node', anonymous=True)
        self.lock = threading.Lock()
        self.current_pose = None
        self.test_mode = test_mode  # Can be False, 'print', 'ori', 'translation', 'move_step', 'translation_smooth', 'ros_tele', or 'ros_hand_test'
        ...
        if self.test_mode == 'ros_tele':
            rospy.Subscriber('/teleop/left_wrist_pos', PoseStamped, self.teleop_wrist_callback)
            rospy.Subscriber('/teleop/left_hand_joint', Float64MultiArray, self.teleop_hand_callback)
            rospy.loginfo("[ROS_TELE] Subscribed to teleop topics: /teleop/left_wrist_pos and /teleop/left_hand_joint")
        elif self.test_mode == 'ros_hand_test':
            rospy.Subscriber('/teleop/left_hand_joint', Float64MultiArray, self.teleop_hand_callback)
            rospy.loginfo("[ROS_HAND_TEST] Subscribed to teleop hand topic: /teleop/left_hand_joint")

        # Camera -> base transform (provided transformation matrix)
        ...
        self.R_hand_y_90 = np.array([
            [ 0,  0,  1],
            [ 0,  1,  0],
            [-1,  0,  0]
        ])
        self.R_hand_x_minus_90 = np.array([
            [ 1,  0,  0],
            [ 0,  0,  1],
            [ 0, -1,  0]
        ])
        ...
        self.R_hand_to_ee = np.dot(self.R_hand_y_90, self.R_hand_x_minus_90)
        ...
        # Inspire Hand serial setup (left hand) - enable for translation mode too
    ...
    def run(self):
        ...
        if self.test_mode == 'print':
            ...
            rospy.loginfo("[ROS_TELE] Topics: /teleop/left_wrist_pos (PoseStamped) and /teleop/left_hand_joint (Float64MultiArray)")
            ...
            while not rospy.is_shutdown():
                ...
                if wrist_data is not None and hand_data is not None:
                    ...
                    print(f"[DEBUG] hand_vals: {hand_vals}")
                    ...
                    if wrist_data is None:
                        rospy.logwarn_throttle(2.0, "[ROS_TELE] No wrist data received on /teleop/left_wrist_pos")
                    if hand_data is None:
                        rospy.logwarn_throttle(2.0, "[ROS_TELE] No hand data received on /teleop/left_hand_joint")
            ...
            rospy.loginfo("[ROS_HAND_TEST] Topic: /teleop/left_hand_joint (Float64MultiArray)")
            ...
            while not rospy.is_shutdown():
                ...
                if hand_data is not None:
                    ...
                    rospy.logwarn_throttle(2.0, "[ROS_HAND_TEST] No hand data received on /teleop/left_hand_joint")
            ...
            filepath = '/home/nvidia/ke/r1_pro_sdk/install/share/mobiman/script/left/robot_commands_left_hand_translate.csv'
            # filepath = '/home/nvidia/ke/r1_pro_sdk/install/share/mobiman/script/left/robot_commands_left_hand.csv'
            ...
            filepath = '/home/nvidia/ke/r1_pro_sdk/install/share/mobiman/script/left/left.csv'
            if not os.path.exists(filepath):
                rospy.logerr(f"Trajectory file not found: {filepath}")
                return
...
if __name__ == '__main__':
    try:
        ...
        # test_mode = 'move_step'  # Change this to False, 'print', 'ori', 'translation', 'move_step', 'translation_smooth', 'ros_tele', or 'ros_hand_test'
        test_mode = False
        ...
        if test_mode == 'print':
            ...
            print("  - /teleop/left_wrist_pos (PoseStamped) for wrist position/orientation")
            print("  - /teleop/left_hand_joint (Float64MultiArray) for hand joint values")
