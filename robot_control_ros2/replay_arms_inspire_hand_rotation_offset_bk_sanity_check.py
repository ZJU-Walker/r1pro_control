...
def publish_frame(node, idx, wrist_pub_L, wrist_pub_R, hand_pub_L, hand_pub_R):
    ...
    # 1. Extract original base position and orientation
    p_base = np.array([
        float(frame['right_pos'][0]),
        float(frame['right_pos'][1]),
        float(frame['right_pos'][2])
    ])
    ...
    R_base = R.from_quat(right_quat).as_matrix()

    # 2. Apply the 30-degree X-axis tilt to orientation (Local Rotation)
    R_tilt = R.from_euler('x', 30.0, degrees=True).as_matrix()
    R_base_new = R_base @ R_tilt

    # 3. Apply translational offset in the NEW tilted local frame
    # (Adjust these [-0.07, 0.06, 0.09] values to match your specific hardware offset)
    local_offset = np.array([-0.07, 0.06, 0.09]) 
    p_base_new = p_base + (R_base_new @ local_offset)

    # 4. Convert the new rotation matrix back to a quaternion
    quat_new = R.from_matrix(R_base_new).as_quat()

    # 5. Assign updated values to the Pose message
    pose_R.pose.position.x = p_base_new[0]
    pose_R.pose.position.y = p_base_new[1]
    pose_R.pose.position.z = p_base_new[2]

    pose_R.pose.orientation.x = quat_new[0]
    pose_R.pose.orientation.y = quat_new[1]
    pose_R.pose.orientation.z = quat_new[2]
    pose_R.pose.orientation.w = quat_new[3]
