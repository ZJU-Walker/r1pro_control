...
class PolicyEvaluator(Node):
    def __init__(self):
        ...
        self.get_logger().info("Policy Evaluator initialized for both arms.")
    ...
    def load_initial_state(self, task_name):
        """Loads initial state for both arms from CSV file."""
        ...
        try:
            with open(csv_filepath, 'r') as f:
                ...
                # Load RIGHT arm state
                right_hand_joints = [float(row[f'right_hand_{i}']) for i in range(20)]
                ...
                # Load LEFT arm state
                left_hand_joints = [float(row[f'left_hand_{i}']) for i in range(20)]
            ...
            self.get_logger().info(f"Successfully loaded initial state for both arms - task: {task_name}")
            return True
    ...
    def reset_to_initial_state(self):
        """Resets both arms and hands to initial state."""
        ...
        self.get_logger().info("Resetting both arms to initial state...")
        ...
        hand_traj_right = JointTrajectory()
        hand_traj_right.header.stamp = now
        hand_traj_right.joint_names = EXPECTED_RIGHT_JOINT_ORDER
        traj_point_right = JointTrajectoryPoint(
            positions=self.initial_state_right['hand_joints'],
            time_from_start=Duration(sec=1, nanosec=0)
        )
        hand_traj_right.points.append(traj_point_right)
        self.hand_pub_right.publish(hand_traj_right)
        ...
        hand_traj_left = JointTrajectory()
        hand_traj_left.header.stamp = now
        hand_traj_left.joint_names = EXPECTED_LEFT_JOINT_ORDER
        traj_point_left = JointTrajectoryPoint(
            positions=self.initial_state_left['hand_joints'],
            time_from_start=Duration(sec=1, nanosec=0)
        )
        hand_traj_left.points.append(traj_point_left)
        self.hand_pub_left.publish(hand_traj_left)

        time.sleep(2.0)  # Wait for robot to reach initial position
        self.get_logger().info("Reset complete for both arms. Ready for evaluation.")
