...
def glove_to_xhand_control(glove_joints):
    ...
    q_xhand[0] = glove_joints[0] * 1.95 + (40 - glove_joints[1]) * 1.8  # thumb rotate
    q_xhand[1] = glove_joints[2] * 2.5         # thumb bend1
    q_xhand[2] = glove_joints[3] * 1.1         # thumb bend2

    q_xhand[3] = (glove_joints[4] - 5)         # index bend1
...
class ManusToXHandright(Node):
    def __init__(self):
        super().__init__('manus_to_xhand_right')
        ...
        # Pub: XHand right command as JointState (expects 12 positions)
        ...
        self.get_logger().info('✅ Manus→XHand right bridge initialized')
        self.get_logger().info('Subscribing: /manus_glove_1')
        self.get_logger().info('Publishing:  /xhand/right/control (JointState, 12 positions)')
