#!/usr/bin/env python3
"""
ROS2 Node for Manus Glove to Inspire Left Hand Control

Subscribes to Manus glove data and publishes commands to Inspire hand.
Topic: /teleop/inspire_left_command (Float32MultiArray)
...
def qd_to_inspire_command(qd):
    ...
    thumb_rotate = (1000 - abs(qd[0]) / 40 * 1000)*0.9
    thumb_bend = (1000 - abs(qd[3]) / 40.0 * 1000)
    index_val = 1000 - abs(qd[6]) * scale - 50
...
class ManusInspireLeftControlNode(Node):
    def __init__(self):
        super().__init__('manus_inspire_left_control_node')

        # Create subscriber for Manus glove data (left glove)
        ...
        # Create publisher for Inspire left hand command
        self.command_publisher = self.create_publisher(
            Float32MultiArray,
            '/teleop/inspire_left_command',
            1
        )
        ...
        self.get_logger().info('Manus Inspire Left Control Node initialized')
        self.get_logger().info(f'User: {USER_NAME}')
        self.get_logger().info('Subscribing to: /manus_glove_1')
        self.get_logger().info('Publishing to: /teleop/inspire_left_command')
    ...
    def glove_callback(self, msg: ManusGlove):
        ...
        try:
            ...
            # Print for debugging
            print(f'Inspire left cmd: pinky={inspire_cmd[0]:.0f}, ring={inspire_cmd[1]:.0f}, '
...
def main(args=None):
    ...
    try:
        node = ManusInspireLeftControlNode()
        rclpy.spin(node)
